...
public class WarningCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String[] strings) {
        ...
        razon.setLength(0);
        if(strings.length>1){
            for(int i = 1;i<strings.length;i++){ razon.append(" ");razon.append(strings[i]);}
        }
        if (strings.length < 2) {
            ...
            if(player == null && !ListWarn.containsKey(strings[0])){
                ...
                pl.getServer().broadcastMessage(WHITE +"El jugador " + GREEN + offlinePlayer.getName() + WHITE +
                        " ha sido advertido por " + GREEN + pl.getName() + WHITE + " por " + GREEN + razon);
                return true;
            }
            ...
            pl.sendMessage(ChatColor.DARK_RED +"Error el jugador no ha sido baneado ya que no se encuentra registrado en el servidor");
        return false;
    }
}